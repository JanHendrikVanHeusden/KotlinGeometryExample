...
internal class GsonSafeParallelogramTest {
    ...
    fun `verify that side lengths are validated on construction`() {
        // OK: all inputs 0
        GsonSafeParallelogram(0.0, 0.0, 0.0)
        // OK: very large values
        GsonSafeParallelogram(1.8e58, 0.0, 88.0)
        ...
        var message = assertFailsWith<IllegalArgumentException> { GsonSafeParallelogram(s1, s2, 0.0) }
            .message
        // kotlin.test Assertions
        ...
        // AssertJ
        assertThatThrownBy { GsonSafeParallelogram(s1, s2, 0.0) }
        ...
        s1 = 50.0
        s2 = -0.00000003
        message = assertFailsWith<IllegalArgumentException> { GsonSafeParallelogram(s1, s2, 0.0) }
                .message
        assertThat(message).isEqualTo(expectedMessage())
    ...
    fun `verify that angle is validated on construction`() {
        // OK: angle 0
        GsonSafeParallelogram(1.0, 2.0, 0.0)
        // OK: angle 90
        GsonSafeParallelogram(1.8e58, 0.0, 90.0)
        ...
        var message = assertFailsWith<IllegalArgumentException> { GsonSafeParallelogram(1.0, 2.0, angle) }
                .message
        assertThat(message).isEqualTo(expectedMessage())

        angle = 90.00000000001
        message = assertFailsWith<IllegalArgumentException> { GsonSafeParallelogram(15.0, 38.4, angle) }
                .message
        assertThat(message).isEqualTo(expectedMessage())

        angle = 361.0
        message = assertFailsWith<IllegalArgumentException> { GsonSafeParallelogram(24.8, 11.2, angle) }
                .message
        assertThat(message).isEqualTo(expectedMessage())
    }
    ...
    @Test
    fun getAngleRadians() {
        val parallelogram1 = GsonSafeParallelogram(s1 = 10.0, s2 = 15.0, angleDegrees = 45.0)
        assertThat(parallelogram1.angleRadians).isCloseTo(Math.PI/4.0, offset(0.00001))

        val parallelogram2 = GsonSafeParallelogram(s1 = 4.3564, s2 = 125.68, angleDegrees = 30.0)
    ...
    fun getLength() {
        var s1 = 10.0
        var s2 = 15.0
        val parallelogram1 = GsonSafeParallelogram(s1, s2, angleDegrees = 76.4)
        assertThat(parallelogram1.length).isEqualTo(s1) // s1 is synonym for length

        s1 = 22.458
        s2 = 314543.12
        val parallelogram2 = GsonSafeParallelogram(s1, s2, angleDegrees = 66.832)
    ...
    fun getWidth() {
        ...
        var s1 = 35.0
        var s2 = 5.0
        var angle = atan(4.0 / 3.0).radiansToDegrees()
        val parallelogram1 = GsonSafeParallelogram(s1, s2, angle)
        assertThat(parallelogram1.width).isCloseTo(4.0, offset(0.00001))

        s1 = 35.0
        s2 = 5.0
        angle = 90.0
        val parallelogram2 = GsonSafeParallelogram(s1, s2, angle)
        ...
        val parallelogram3 = GsonSafeParallelogram(s1, s2, angle)
        assertThat(parallelogram3.width).isEqualTo(0.0) // because angle = 0 degrees (flat)

        s1 = 4.389
        s2 = 17.0
        angle = 30.0
        val parallelogram4 = GsonSafeParallelogram(s1, s2, angle)
        ...
        val parallelogram5 = GsonSafeParallelogram(s1, s2, angle)
    ...
    fun getArea() {
        ...
        val spyk1 = spyk(GsonSafeParallelogram(s1, s2, angleDegrees), recordPrivateCalls = true)
        ...
        s2 = 85.4
        angleDegrees = 14.37
        val parallelogram2 = GsonSafeParallelogram(s1, s2, angleDegrees)
    ...
    fun testToString() {
        ...
        val parallelogram1 = GsonSafeParallelogram(s1, s2, angleDegrees)
    ...
    fun testEquals() {
        // given
        val parallelogram1 = GsonSafeParallelogram(s1 = 10.0, s2 = 15.0, angleDegrees = 25.0)
        val parallelogram2 = GsonSafeParallelogram(s1 = 10.0, s2 = 15.0, angleDegrees = 25.0)
        // check equality
        assertThat(parallelogram1).isEqualTo(parallelogram2)

        // given
        var parallelogramDifferent = parallelogram1.copy(s1 = 10.0001)
        // check inequality
        assertThat(parallelogramDifferent).isNotEqualTo(parallelogram1)
        // given
        parallelogramDifferent = parallelogram1.copy(s2 = 8.0)
        // check inequality
        assertThat(parallelogramDifferent).isNotEqualTo(parallelogram1)
        // given
        parallelogramDifferent = parallelogram1.copy(angleDegrees = 33.0)
        // check inequality
        assertThat(parallelogramDifferent).isNotEqualTo(parallelogram1)
    }
    ...
    fun `equals call should not call lazy attributes`() {
        ...
        val spykDifferent = spyk(parallelogram1.copy(s1 = 8.0))
